...
const PUBLIC_DOCUMENT_CONTEXTS = {
  ...
  candidate_registration: {
    ...
    allowedDocumentTypes: new Set(['passport', 'right_to_work', 'visa_permit', 'cv']),
    invalidDocumentTypeMessage: 'Only CV, passport, right-to-work, or visa / permit evidence can be uploaded from the registration form.',
...
function validateDocumentRequest({ fileName, mimeType, sizeBytes }) {
  ...
  if (!ALLOWED_DOCUMENT_EXTENSIONS.includes(extension)) {
    throw coded(400, 'Upload a PDF, Word document, or supported image file such as JPG, PNG, WEBP, HEIC, or TIFF.', 'application_document_type_invalid');
  }
...
function buildDocumentPayload(candidate, linkedApplication, body, validated, storagePath, now, context = resolvePublicDocumentContext(body)) {
  ...
  return {
    ...
    meta: withDocumentVerificationMeta(documentType, {
      ...
      right_to_work_evidence_type: trimString(body.right_to_work_evidence_type, 80) || null,
