...
function normaliseSupabaseRows(data = [], baseWeekEnding = DEFAULT_SETTINGS.fiscal_week1_ending) {
  return data.map((row) => {
    ...
    const payrollMeta = typeof row.payroll_meta === 'object' && row.payroll_meta ? row.payroll_meta : {};
    const payrollStatusRaw = row.payroll_status || row.pay_status || payrollMeta.status || null;
    const paidAt = row.paid_at || row.payment_paid_at || payrollMeta.paid_at || null;
    const payrollBatch = row.payroll_batch || payrollMeta.batch || null;
    const paymentReference = row.payment_ref || payrollMeta.reference || null;
    ...
    return {
      ...
      payroll_status: payrollStatusRaw ? String(payrollStatusRaw).toLowerCase() : (row.status || '').toLowerCase(),
      paid_at: paidAt,
...
function normaliseStaticRows(rows = [], baseWeekEnding = DEFAULT_SETTINGS.fiscal_week1_ending) {
  return rows.map((row) => ({
    ...
    charge_amount: row.charge_amount,
    gp_amount: row.gp_amount,
    payroll_status: (row.payroll_status || row.status || 'draft').toLowerCase(),
    paid_at: row.paid_at || row.payment_paid_at || null,
    payroll_batch: row.payroll_batch || null,
    payment_reference: row.payment_reference || row.payrollRef || null,
