...
function resolvePathFromEvent(event) {
  ...
  if (!pathname && event && event.path) {
    pathname = event.path;
  }
  if (!pathname) return '';

  const match = pathname.match(/\/.netlify\/functions\/identity-proxy\/?(.*)$/i);
  if (!match) return '';
  const splat = match[1] || '';
  try {
...
exports.handler = async (event) => {
  ...
  const proxyPath = singleParams?.path || resolvePathFromEvent(event) || '';
