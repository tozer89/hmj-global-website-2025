...
const baseHandler = async (event, context) => {
  try {
    ...
    const slug = normaliseSlug(stem, stem, 'notice-image');
    const now = new Date();
    const year = String(now.getUTCFullYear());
    const month = String(now.getUTCMonth() + 1).padStart(2, '0');
    const key = `noticeboard/${year}/${month}/${Date.now()}-${randomUUID().slice(0, 8)}-${slug}.${extension}`;

    const storage = supabase.storage.from(NOTICEBOARD_BUCKET);
    ...
    if (replaceStorageKey && replaceStorageKey !== key) {
      ...
      if (replaceError) {
        console.warn('[noticeboard-image-upload] unable to replace prior image %s', replaceError.message || replaceError);
      }
    }
    ...
    return {
      ...
      body: JSON.stringify({
        ok: false,
        error: bucketMissing
          ? 'Noticeboard image storage is not ready in this environment yet.'
          : message,
