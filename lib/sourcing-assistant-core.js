...
function hasLeadershipSignal(candidate = {}, text = '') {
  const titleContext = flattenText([
    candidate.current_title,
    candidate.headline,
  ]);
  if (/\b(?:manager|lead|supervisor|director|head)\b/i.test(titleContext)) {
    return true;
  }

  const summaryContext = flattenText([
    candidate.summary_text,
    text,
  ]);
  return /\b(?:led|leading|manage(?:d|s|ing)?|oversaw|oversee|owned|owning|directed|responsible for)\b/i.test(summaryContext);
}
...
function bulkCvOcrEnabled() {
  return hasUsableOpenAiApiKey(process.env.OPENAI_API_KEY);
}
...
function scorePreviewCandidate(job, candidate = {}, roleConfig = buildDefaultRoleConfig()) {
  ...
  if (containsPhrase(previewText, 'senior') || hasLeadershipSignal(candidate, previewText)) {
    totalScore += 6;
...
function reviewFullCv(job, candidate, cvText, roleConfig = buildDefaultRoleConfig()) {
  ...
  if (hasLeadershipSignal(candidate, sourceText)) {
    score += 8;
...
async function importBulkCvFiles({
  workflowRoot,
  roleId,
  files = [],
  enablePdfOcr = null,
  ...
  const fileResults = [];
  const ocrEnabled = enablePdfOcr === true
    || (enablePdfOcr !== false && (bulkCvOcrEnabled() || typeof ocrFetchImpl === 'function'));
  ...
  for (let index = 0; index < inputFiles.length; index += 1) {
    ...
    if (!BULK_CV_ALLOWED_EXTENSIONS.has(prepared?.extension || extension)) {
      fileResults.push(buildBulkCvUploadFileResult({
        ...
        errorMessage: 'Only PDF, DOCX, and legacy DOC bulk parsing are supported in this uploader.',
      }));
      continue;
    }
    ...
    try {
      extractionResult = await candidateMatcherCore.extractCandidateDocuments([prepared], {
        ...
        ocrFetchImpl: typeof ocrFetchImpl === 'function' ? ocrFetchImpl : undefined,
      });
    } catch (error) {
      fileResults.push(buildBulkCvUploadFileResult({
        fileName: originalFileName,
