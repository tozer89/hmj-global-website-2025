...
function calculateRankScore(record, roleConfig = buildDefaultRoleConfig()) {
  ...
  return Number((baseScore + stagePriority(record?.lifecycle?.current_stage) + shortlistBoost + draftBoost + emailPenalty).toFixed(2));
}
...
function applyCandidateRanking(candidateRecords, roleConfig = buildDefaultRoleConfig()) {
  ...
  candidateRecords.forEach((record) => {
    record.ranking = {
      ...
      total_score: calculateRankScore(record, roleConfig),
      pinned: ['strong_shortlist', 'outreach_ready', 'outreach_drafted', 'contacted', 'awaiting_reply'].includes(record.lifecycle.current_stage),
...
function buildShortlistProgress(roleConfig, metrics) {
  ...
  const strong = metrics.lifecycle_counts.strong_shortlist || 0;
  const possible = metrics.lifecycle_counts.possible_shortlist || 0;
...
function formatSourceAuditString(record) {
  if (trimString(record?.source_audit?.source_url, 1200)) {
    return record.source_audit.source_url;
  }
  ...
  return `Found from ${record?.source || 'the source'}${foundAt ? ` on ${foundAt}` : ''}${searchVariant ? ` using ${searchVariant}` : ''}${booleanUsed ? ` using Boolean: ${booleanUsed}` : ''}`.trim();
}
...
function summariseDashboard(job, metrics, searchPack, candidateRecords, roleDir, roleConfig = buildDefaultRoleConfig()) {
  ...
  const roleArtifacts = {
    intake: buildArtifactDescriptor(roleDir, `${ROLE_INPUTS_DIR}/${DEFAULT_ROLE_FILE}`, { label: 'Job Spec Intake', downloadable: true }),
    roleConfig: buildArtifactDescriptor(roleDir, `${ROLE_INPUTS_DIR}/${DEFAULT_ROLE_CONFIG_FILE}`, { label: 'Role Config', downloadable: true }),
    candidates: buildArtifactDescriptor(roleDir, `${ROLE_INPUTS_DIR}/${DEFAULT_CANDIDATES_FILE}`, { label: 'Imported Previews', downloadable: true }),
    candidatesCsv: buildArtifactDescriptor(roleDir, `${ROLE_INPUTS_DIR}/${DEFAULT_CANDIDATES_CSV_FILE}`, { label: 'Imported Previews CSV', downloadable: true }),
    operatorOverrides: buildArtifactDescriptor(roleDir, `${ROLE_INPUTS_DIR}/${DEFAULT_OPERATOR_OVERRIDES_FILE}`, { label: 'Operator Overrides', downloadable: true }),
    searchPackMarkdown: buildArtifactDescriptor(roleDir, `${ROLE_OUTPUTS_DIR}/search-pack.md`, { label: 'Search Pack', downloadable: true }),
    searchPackJson: buildArtifactDescriptor(roleDir, `${ROLE_OUTPUTS_DIR}/search-pack.json`, { label: 'Search Pack JSON', downloadable: true }),
    previewTriage: buildArtifactDescriptor(roleDir, `${ROLE_OUTPUTS_DIR}/preview-triage.json`, { label: 'Preview Triage', downloadable: true }),
    candidateRecords: buildArtifactDescriptor(roleDir, `${ROLE_OUTPUTS_DIR}/candidate-records.json`, { label: 'Candidate Records', downloadable: true }),
    candidateExportCsv: buildArtifactDescriptor(roleDir, `${ROLE_OUTPUTS_DIR}/${DEFAULT_CANDIDATE_EXPORT_FILE}`, { label: 'Candidate Export CSV', downloadable: true }),
    metrics: buildArtifactDescriptor(roleDir, `${ROLE_OUTPUTS_DIR}/metrics.json`, { label: 'Metrics JSON', downloadable: true }),
    metricsSummary: buildArtifactDescriptor(roleDir, `${ROLE_OUTPUTS_DIR}/metrics-summary.md`, { label: 'Metrics Summary', downloadable: true }),
    operatorReview: buildArtifactDescriptor(roleDir, `${ROLE_OUTPUTS_DIR}/operator-review.md`, { label: 'Operator Review', downloadable: true }),
    runSummary: buildArtifactDescriptor(roleDir, `${ROLE_OUTPUTS_DIR}/${DEFAULT_RUN_SUMMARY_FILE}`, { label: 'Run Summary', downloadable: true }),
    dashboardSummary: buildArtifactDescriptor(roleDir, `${ROLE_OUTPUTS_DIR}/dashboard-summary.json`, { label: 'Dashboard Summary', downloadable: true }),
    auditLog: buildArtifactDescriptor(roleDir, `${ROLE_OUTPUTS_DIR}/audit-log.jsonl`, { label: 'Audit Log', downloadable: true }),
  };
  ...
  return {
    ...
    candidateDetails: rankedCandidates.map((record) => ({
      ...
      artifacts: {
        candidateRecord: buildArtifactDescriptor(roleDir, `${ROLE_RECORDS_DIR}/${record.candidate_id}.json`, { label: 'Candidate Record', downloadable: true }),
        cvFile: buildArtifactDescriptor(roleDir, record.full_cv.cv_file || '', { label: 'CV File', downloadable: true }),
        outreachDraft: buildArtifactDescriptor(roleDir, record.outreach.draft_path || '', { label: 'Outreach Draft', downloadable: true }),
      },
    })),
    drafts,
...
function importPreviewCandidates({ workflowRoot, roleId, inputPath }) {
  ...
  const importMethod = trimString(path.extname(resolvedInputPath).slice(1).toLowerCase(), 12) === 'csv'
    ? 'csv_import'
    : 'json_import';
  ...
  const importedCandidates = parseCandidateImportText(resolvedInputPath, readTextFile(resolvedInputPath))
    .map((candidate) => normaliseCandidateInput({
      ...candidate,
      imported_at: candidate.imported_at || importTimestamp,
  ...
  importedCandidates.forEach((candidate, index) => {
    ...
    mergedById.set(candidateId, normaliseCandidateInput({
      ...existing,
      ...candidate,
      candidate_id: candidateId,
      imported_at: candidate.imported_at || existing.imported_at || importTimestamp,
      import_method: candidate.import_method || existing.import_method || importMethod,
    }));
  });
...
async function runRoleWorkspaceInternal({ workflowRoot, roleId, action = 'run_all', roleDir = null }) {
  ...
  writeJsonFile(path.join(resolvedRoleDir, ROLE_OUTPUTS_DIR, 'dashboard-summary.json'), dashboardSummary);
