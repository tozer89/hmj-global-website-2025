...
const LIKELY_ROLE_TITLE_PATTERN = /\b(?:manager|engineer|electrician|improver|mate|supervisor|director|planner|coordinator|consultant|technician|lead|estimator|buyer|surveyor)\b/i;
...
function cleanRoleTitle(value) {
  ...
  const roleMatch = text.match(/\b([A-Z][A-Za-z&/+-]*(?:\s+[A-Z][A-Za-z&/+-]*){0,6}\s(?:Manager|Engineer|Electrician|Improver|Mate|Supervisor|Director|Planner|Coordinator|Consultant|Technician|Lead|Estimator|Buyer|Surveyor|Designer|Specialist|Foreman))\b/i);
  if (roleMatch) {
...
function collectProjectEvidenceFromHistory(entries) {
  ...
  (Array.isArray(entries) ? entries : []).forEach((entry) => {
    splitIntoSentences(entry?.summary || '').forEach((sentence) => {
      ...
      if (!cleaned) return;
      if (!/\b(data centre|mission critical|cleanroom|pharma|commissioning|brownfield|fit-out|mep|csa|commercial)\b/i.test(cleaned)) {
        return;
      }
...
function collectRelevantProjects(text, jobSpecText, employmentHistory = [], context = {}) {
  ...
  const lines = splitNonEmptyLines(text).filter((line) => {
    ...
    if (/\b(?:degree|bsc|msc|certificate|qualification|accreditation)\b/i.test(line)) return false;
  ...
  return filterClientFacingItems(lines.concat(summaryEvidence), 5, 220);
}
...
function collectEmploymentHistory(text) {
  ...
  lines.forEach((line) => {
    ...
    if (pipeSegments.length >= 2 && looksLikeDateRange(pipeSegments[pipeSegments.length - 1])) {
      ...
      current = {
        ...
        title: trimString(pipeSegments.slice(0, -1).join(' | '), 140),
        company: '',
        summary: '',
        bullets: [],
      };
    ...
    const titleAndDateMatch = normalisedLine.match(/^(.*?)(?:\s*[|]\s*|\s+-\s+)((?:[A-Z][a-z]{2,8}\s+)?\d{4}\s*[–-]\s*(?:(?:[A-Z][a-z]{2,8}\s+)?(?:Present|\d{4})))$/i);
    if (titleAndDateMatch) {
      if (current) entries.push(current);
      current = {
        dates: trimString(titleAndDateMatch[2], 120),
        title: trimString(titleAndDateMatch[1], 140),
        company: '',
        summary: '',
        bullets: [],
      };
    ...
    if (current && current.title && current.summary && isLikelyRoleTitle(line)) {
    ...
    if (/^[•*-]\s*/.test(line)) {
      current.bullets.push(cleanClientFacingText(line.replace(/^[•*-]\s*/, ''), 220));
      return;
    }
    current.summary = trimString(fixCommonExtractionArtifacts(`${current.summary} ${line}`), 900);
  });
  if (current) entries.push(current);
  ...
  return entries
    .map((entry) => ({
      ...
      company: trimString(cleanClientFacingText(entry.company, 160), 160),
      summary: trimString(splitIntoSentences(entry.summary).slice(0, 3).join(' '), 700),
      bullets: filterClientFacingItems(entry.bullets, 4, 220),
    }))
...
function finaliseStructuredProfile(profile, context = {}) {
  ...
  const location = sanitizeLocation(profile.location, formatOptions.anonymiseMode);
...
function buildFallbackProfile({ candidateText, jobSpecText, candidateFileName, options = {} }) {
  ...
  const candidateName = guessCandidateName(candidateFileName, preparedCandidateText);
  ...
  const employmentHistory = collectEmploymentHistory(preparedCandidateText);
  ...
  const displayLocation = safeLocation || guessFallbackLocation(preparedCandidateText, {
    candidateName,
    role: profileRole,
        ...
        content: [
          {
            ...
            text: [
              ...
              '- profile: one polished paragraph, 90 to 150 words, written for a client and easy to scan.',
...
function createBrandedHeader(logoBuffer) {
      ...
      new Paragraph({
        ...
        children: [
          new ImageRun({
            ...
            transformation: { width: 112, height: 78 },
          }),
        ],
      })
    );
  ...
  children.push(
    new Paragraph({
      ...
      children: [
        new TextRun({
          text: 'HMJ Global | Client-ready candidate profile',
          bold: true,
          color: BRAND_PRIMARY_COLOR,
          font: BRAND_FONT,
          size: 20,
...
function createBrandedFooter(logoBuffer) {
  const children = [
    new Paragraph({
...
function appendEmploymentHistorySection(children, entries) {
  ...
  entries.forEach((entry) => {
    if (entry.title) children.push(textParagraph(entry.title, { bold: true, size: 26, spacing: { after: 30 } }));
    ...
    if (entry.summary) children.push(textParagraph(entry.summary, { size: 24, spacing: { after: 80, line: 330 } }));
...
async function buildClientReadyDocx(profile, options = {}) {
  ...
  const defaultHeader = createBrandedHeader(logoBuffer);
  const defaultFooter = createBrandedFooter(logoBuffer);
