...
function guessCandidateName(fileName, text) {
  ...
  if (/^[A-Z][a-z]+(?:\s+[A-Z][A-Za-z.'-]+){1,3}$/.test(firstLine)) return firstLine;
  return '';
}
...
function fallbackProfileParagraph(text) {
  ...
  if (current.length) paragraphs.push(current.join(' '));
  const selected = paragraphs.find((paragraph) => paragraph.length > 180) || paragraphs[0] || lines.slice(0, 6).join(' ');
...
function collectKeySkills(text) {
  ...
  const explicit = cleanList(section, 14, 180).filter((line) => !isLikelyTemplateArtifact(line));
...
function buildFallbackProfile({ candidateText, jobSpecText, candidateFileName, options = {} }) {
  ...
  return sanitiseStructuredProfile({
    ...
    sanitized_location: safeLocation,
...
async function callOpenAiFormatter({
  ...
  const formatOptions = normaliseFormatOptions(options);
  const model = trimString(process.env.OPENAI_CV_FORMAT_MODEL, 120) || DEFAULT_MODEL;
  const requestBody = {
    model,
...
function buildCoverTable(profile, options = {}) {
  const formatOptions = normaliseFormatOptions(options);
  const rows = formatOptions.coverPageMode === 'condensed'
    ? [
        coverRow('Position', profile.targetRole),
        coverRow('Candidate ID', profile.candidateReference),
        coverRow('Location', profile.location),
        coverRow('Availability', profile.interviewAvailability),
        coverRow('Key Qualifications', profile.qualifications.concat(profile.accreditations), { maxItems: 4 }),
      ]
    : [
        coverRow('Position', profile.targetRole),
        coverRow('Candidate ID', profile.candidateReference),
        coverRow('Location', profile.location),
        coverRow('Relevant Data Centre Projects / Experience', profile.relevantProjects, { maxItems: 4 }),
        coverRow('Qualifications & Accreditations', profile.qualifications.concat(profile.accreditations), { maxItems: 5 }),
        coverRow('Languages', profile.languages, { maxItems: 4 }),
        coverRow('Availability to Interview', profile.interviewAvailability),
      ];
...
async function formatClientReadyCv({
  ...
  if (formatOptions.preferAiAssist !== false) {
    ...
    if (ai.ok) {
      ...
      warnings.push(`AI formatter fallback used: ${aiMessage}.`);
    }
  }
