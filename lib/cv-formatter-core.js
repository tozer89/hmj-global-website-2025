...
const { hasUsableOpenAiApiKey } = require('./openai-env.js');
...
const COMMON_SECTION_HEADINGS = [
  ...
  'areas of experience and expertease',
...
const LIKELY_ROLE_TITLE_PATTERN = /\b(?:manager|engineer|electrician|improver|mate|supervisor|director|planner|coordinator|consultant|technician|lead|estimator|buyer|surveyor|designer|foreman|person|authority|inspector|officer|controller|auditor|specialist|apprentice|maintenance)\b/i;
const ROLE_TITLE_CAPTURE_PATTERN = /\b([A-Z][A-Za-z0-9&/+-]*(?:\s+[A-Z][A-Za-z0-9&/+-]*){0,7}\s(?:Manager|Engineer|Electrician|Improver|Mate|Supervisor|Director|Planner|Coordinator|Consultant|Technician|Lead|Estimator|Buyer|Surveyor|Designer|Foreman|Person|Authority|Inspector|Officer|Controller|Auditor|Specialist|Apprentice|Maintenance))\b/gi;
...
function cleanClientFacingText(value, maxLength = 1400) {
  const text = fixCommonExtractionArtifacts(value).replace(/^[•*-]\s*/, '');
...
function guessCandidateName(fileName, text) {
  ...
  const candidateLine = lines.find((line, index) => {
    ...
    if (isLikelyTemplateArtifact(line) || looksLikeSectionHeading(line) || isLikelyRoleTitle(line)) return false;
  ...
  const candidateLineLooksLikeName = isLikelyPersonName(candidateLine);
  if (candidateLineLooksLikeName && !fileLooksLikeName) return candidateLine;
  if (firstLineLooksLikeName && !fileLooksLikeName) return firstLine;
  if (fileLooksLikeName) return fromFile;
  if (candidateLineLooksLikeName) return candidateLine;
...
function mergeWrappedLines(lines) {
  const input = Array.isArray(lines) ? lines.map((line) => trimString(line)).filter(Boolean) : [];
  const merged = [];
  input.forEach((line) => {
    ...
    const isContinuation = previous
      ...
      && !/^[•*-]/.test(line);
    if (isContinuation) {
      merged[merged.length - 1] = trimString(`${previous} ${line}`, 320);
      return;
    }
    merged.push(line);
  });
...
function collectHeadingSection(text, headings) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (collected.length) return mergeWrappedLines(collected).join('\n');
  }
  return '';
}
...
function looksLikeDateRange(value) {
  const text = trimString(value);
  if (!text) return false;
  return /(?:\b(?:jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)[a-z]*\b\s*)?\d{4}\s*(?:[–-]|to)\s*(?:(?:\b(?:jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)[a-z]*\b\s*)?(?:pres|present|\d{4}))/i.test(text)
...
function isLikelyTemplateArtifact(value) {
  ...
  return COMMON_SECTION_HEADINGS.includes(text)
    ...
    || text === 'candidate id'
    || text === 'scope & role'
    || text === 'multiple relevant references available on request';
}
...
function extractDateRangeFromText(value) {
  ...
  const patterns = [
    ...
    /\b(?:jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)[a-z]*\s*\d{2,4}\s*(?:[–-]|to)\s*(?:(?:jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)[a-z]*\s*)?(?:pres|present|\d{2,4})\b/i,
    /\b\d{4}\s*(?:[–-]|to)\s*(?:pres|present|\d{4})\b/i,
  ];
...
function findDateRangeMatches(value) {
  ...
  if (!text) return [];
  return Array.from(text.matchAll(/\b(?:(?:\d{1,2}\s*\/\s*)?\d{2,4}|(?:jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)[a-z]*\s*\d{2,4})\s*(?:[–-]|to)\s*(?:pres|present|(?:\d{1,2}\s*\/\s*)?\d{2,4}|(?:(?:jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)[a-z]*\s*\d{2,4}))\b/gi))
    .map((match) => ({
...
function buildInlineEmploymentEntry(value, fallbackTitle = '') {
  ...
  const suffixRole = cleanRoleTitle(suffix);
  const title = trimString(roleMatch?.text || suffixRole || cleanRoleTitle(fallbackTitle), 140);
  let company = prefix;
...
function parseYearBounds(value) {
  ...
  const monthYearMatch = text.match(/\b(?:\d{1,2}\s*\/\s*)?(\d{2,4})\s*(?:[–-]|to)\s*(pres|present|(?:\d{1,2}\s*\/\s*)?(\d{2,4}))\b/i);
  if (monthYearMatch) {
    const start = expandTwoDigitYear(monthYearMatch[1]);
    const hasPresent = /^(?:pres|present)$/i.test(trimString(monthYearMatch[2]));
  ...
  const hasPresent = /\b(?:pres|present)\b/i.test(text);
...
function collectRelevantProjects(text, jobSpecText, employmentHistory = [], context = {}) {
  ...
  const lines = splitNonEmptyLines(text).filter((line) => {
    ...
    if (/^(?:i am|i started|amongst|for more detailed|angus college|dundee college|city & guilds|nationwide platforms)/i.test(line)) return false;
    ...
    if (extractDateRangeFromText(line) && !/\b(data centre|mission critical|semiconductor|mine|quarr|industrial|pharma|commissioning|fit-out|retrofit|bms|switchgear|busbar|ups)\b/i.test(lower)) return false;
...
function collectEmploymentHistory(text) {
  const section = collectHeadingSection(text, ['Employment History', 'Work Experience', 'Experience', 'Professional Experience', 'Career History']);
...
function guessFallbackLocation(text, context = {}) {
  ...
  const line = splitNonEmptyLines(text).find((entry) => {
    ...
    return /^[A-Z][A-Za-z\s,.'-]{1,60}$/.test(entry);
  }) || '';
...
async function verifyPremiumCandidatePack({ profile, buffer, options = {}, requestFetch = fetchImpl }) {
  ...
  const expectedHeadings = [
    'Executive Summary',
    Array.isArray(profile.keySkills) && profile.keySkills.length ? 'Core Strengths' : '',
    Array.isArray(profile.relevantProjects) && profile.relevantProjects.length ? 'Key Project Experience' : '',
    Array.isArray(profile.employmentHistory) && profile.employmentHistory.length ? 'Employment History' : '',
    [].concat(profile.qualifications || [], profile.accreditations || []).length ? 'Qualifications & Certifications' : '',
  ].filter(Boolean);
  ...
  const duplicateLines = splitNonEmptyLines(snapshot.documentText)
    .map((line) => cleanClientFacingText(line, 260))
    .filter((line) => line.length >= 24)
    .filter((line) => !looksLikeSectionHeading(line))
    .filter((line) => !isFooterArtifactText(line))
