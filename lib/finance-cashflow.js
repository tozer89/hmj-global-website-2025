...
const WEEK_COUNT = 13;
const DEFAULT_CASH_BUFFER = 0;
const DEFAULT_RECEIPT_DELAY_DAYS = 0;
const DEFAULT_PAYROLL_COVER_WARNING_WEEKS = 1;
const DEFAULT_CONCENTRATION_WARNING_PERCENT = 40;

const SCENARIO_PRESETS = {
  base: {
    key: 'base',
    label: 'Base',
    description: 'Current HMJ assumptions with no extra stress applied.',
    receiptDelayDays: 0,
    payrollMultiplier: 1,
    advancePercentDelta: 0,
    feePercentDelta: 0,
    settlementLagDaysDelta: 0,
  },
  late_receipts: {
    key: 'late_receipts',
    label: 'Receipts slip',
    description: 'Forecast receipts arrive one week later than expected.',
    receiptDelayDays: 7,
    payrollMultiplier: 1,
    advancePercentDelta: 0,
    feePercentDelta: 0,
  ...
  payroll_pressure: {
    key: 'payroll_pressure',
    label: 'Payroll pressure',
    description: 'Payroll and contractor outflows land 6% above plan.',
    receiptDelayDays: 0,
    payrollMultiplier: 1.06,
    advancePercentDelta: 0,
    feePercentDelta: 0,
    settlementLagDaysDelta: 0,
  },
  funding_squeeze: {
    ...
    description: 'Advance rates soften and finance fees rise slightly.',
    receiptDelayDays: 0,
    payrollMultiplier: 1,
    advancePercentDelta: -5,
    feePercentDelta: 0.5,
    settlementLagDaysDelta: 7,
  },
  tight_cash: {
    key: 'tight_cash',
    label: 'Tight cash',
    description: 'Combines slower receipts, softer funding, and payroll pressure.',
    receiptDelayDays: 7,
    payrollMultiplier: 1.06,
    advancePercentDelta: -5,
    feePercentDelta: 0.5,
...
function normalizeScenarioPreset(value) {
  const raw = lowerText(value, 40);
  if (SCENARIO_PRESETS[raw]) return raw;
  if (['late', 'delayed', 'delayed_receipts', 'receipt_delay'].includes(raw)) return 'late_receipts';
  if (['payroll', 'payroll_stress', 'payroll_uplift'].includes(raw)) return 'payroll_pressure';
  if (['funding', 'funding_stress'].includes(raw)) return 'funding_squeeze';
  if (['tight', 'stress', 'stress_test'].includes(raw)) return 'tight_cash';
  return DEFAULT_SCENARIO_KEY;
}

function getScenarioPresetConfig(value) {
  return SCENARIO_PRESETS[normalizeScenarioPreset(value)] || SCENARIO_PRESETS.base;
}

function average(values = []) {
  const filtered = values.filter((value) => Number.isFinite(toNumber(value, NaN)));
  if (!filtered.length) return 0;
  return roundMoney(filtered.reduce((sum, value) => sum + toNumber(value, 0), 0) / filtered.length);
}

function isPayrollCategory(value) {
  const raw = lowerText(value, 80);
  return raw.includes('payroll') || raw.includes('contractor') || raw.includes('wages');
}
...
function addExposureEntry(exposureMap, customerName, amount, options = {}) {
  const name = trimString(customerName, 240) || 'Unallocated customer';
  const key = lowerText(name, 240) || 'unallocated';
  ...
  existing.totalExposure = roundMoney(existing.totalExposure + roundMoney(amount));
  if (options.overdue) existing.overdueExposure = roundMoney(existing.overdueExposure + roundMoney(amount));
  if (options.funded) existing.fundedExposure = roundMoney(existing.fundedExposure + roundMoney(amount));
  else existing.normalExposure = roundMoney(existing.normalExposure + roundMoney(amount));
  existing.invoiceCount += Number(options.invoiceCount || 1);
  if (options.currency) existing.currencies.add(normalizeCurrency(options.currency));
  const nextReceiptDate = normalizeDate(options.nextReceiptDate);
  if (nextReceiptDate && (!existing.nextReceiptDate || compareDates(nextReceiptDate, existing.nextReceiptDate) < 0)) {
    existing.nextReceiptDate = nextReceiptDate;
  }
  exposureMap.set(key, existing);
}

function finalizeExposureEntries(exposureMap) {
  const rows = Array.from(exposureMap.values()).map((row) => ({
    ...row,
    currencies: Array.from(row.currencies.values()),
  }));
  const totalExposure = rows.reduce((sum, row) => sum + roundMoney(row.totalExposure), 0);
  return rows
    .map((row) => ({
      ...row,
      shareOfExposure: totalExposure > 0 ? roundMoney((row.totalExposure / totalExposure) * 100) : 0,
    }))
...
function addInvoiceCashLines({ invoice, fundingRules, customers, assumptions, weeks, source, actual, scenario = SCENARIO_PRESETS.base }) {
  ...
  const baseDueDate = normalizeDate(
  ...
  const receiptDelayDays = actual
    ? 0
    : Math.max(
      0,
      Math.round(
        toNumber(assumptions.receipt_delay_days ?? assumptions.receiptDelayDays, DEFAULT_RECEIPT_DELAY_DAYS)
        + toNumber(scenario.receiptDelayDays, 0)
      )
    );
  const dueDate = receiptDelayDays > 0 ? addCalendarDays(baseDueDate, receiptDelayDays) : baseDueDate;
  ...
  const advanceDelta = toNumber(scenario.advancePercentDelta, 0);
  const baseAdvancePercent = toNumber(fundingRule.advance_percent ?? fundingRule.advancePercent, 90);
  const baseRetentionPercent = toNumber(fundingRule.retention_percent ?? fundingRule.retentionPercent, Math.max(0, 100 - baseAdvancePercent));
  const advancePercent = clampNumber(baseAdvancePercent + advanceDelta, 0, 100);
  const retentionPercent = clampNumber(baseRetentionPercent - advanceDelta, 0, 100);
  const feePercent = toNumber(fundingRule.fee_percent ?? fundingRule.feePercent, 0) + toNumber(scenario.feePercentDelta, 0);
  const interestPercent = toNumber(fundingRule.interest_percent ?? fundingRule.interestPercent, 0);
  const fundedOnIssue = fundingRule.funded_on_issue !== false;
  const settlementLagDays = Math.max(
    0,
    Math.round(
      toNumber(fundingRule.settlement_lag_days ?? fundingRule.settlementLagDays, 14)
      + toNumber(scenario.settlementLagDaysDelta, 0)
    )
  );
  ...
  const advanceDate = fundedOnIssue ? issueDate : dueDate;

  pushWeekLine(weeks, {
    date: compareDates(advanceDate, weeks[0]?.weekStart) < 0 ? weeks[0]?.weekStart : advanceDate,
...
function summariseInsights({ weeks, summary, assumptions, qboInvoices, qboBills, exposureEntries, scenario }) {
  const today = normalizeDate(new Date().toISOString());
  const minimumCashBuffer = roundMoney(toNumber(
    assumptions.minimum_cash_buffer ?? assumptions.minimumCashBuffer,
    DEFAULT_CASH_BUFFER
  ));
  const payrollCoverWarningWeeks = toNumber(
    assumptions.payroll_cover_warning_weeks ?? assumptions.payrollCoverWarningWeeks,
    DEFAULT_PAYROLL_COVER_WARNING_WEEKS
  ) || DEFAULT_PAYROLL_COVER_WARNING_WEEKS;
  const concentrationWarningPercent = toNumber(
    assumptions.concentration_warning_percent ?? assumptions.concentrationWarningPercent,
    DEFAULT_CONCENTRATION_WARNING_PERCENT
  ) || DEFAULT_CONCENTRATION_WARNING_PERCENT;

  const overdueReceivables = roundMoney((qboInvoices || []).reduce((sum, invoice) => {
    const balance = toNumber(invoice.balance_amount ?? invoice.balanceAmount, 0);
    const dueDate = normalizeDate(invoice.due_date ?? invoice.dueDate);
    if (balance <= 0 || !dueDate || compareDates(dueDate, today) >= 0) return sum;
    return sum + balance;
  }, 0));

  const overduePayables = roundMoney((qboBills || []).reduce((sum, bill) => {
    const balance = toNumber(bill.balance_amount ?? bill.balanceAmount, 0);
    const dueDate = normalizeDate(bill.due_date ?? bill.dueDate);
    if (balance <= 0 || !dueDate || compareDates(dueDate, today) >= 0) return sum;
    return sum + balance;
  }, 0));

  const payrollByWeek = weeks.map((week) => roundMoney(week.lines.reduce((sum, line) => {
    if (line.direction !== 'outflow' || !isPayrollCategory(line.category)) return sum;
    return sum + toNumber(line.amount, 0);
  }, 0)));
  const activePayrollWeeks = payrollByWeek.filter((value) => value > 0);
  const averageWeeklyPayroll = average(activePayrollWeeks.slice(0, 4));
  const nextFourWeekPayroll = roundMoney(payrollByWeek.slice(0, 4).reduce((sum, value) => sum + value, 0));
  const payrollCoverWeeks = averageWeeklyPayroll > 0
    ? roundMoney((summary.currentCash || 0) / averageWeeklyPayroll)
    : 0;

  const strongestExposure = exposureEntries[0] || null;
  const warnings = [];
  const commentary = [];
  const worstWeek = weeks.reduce((lowest, week) => {
  ...
  if (worstWeek) {
    commentary.push({
      tone: worstWeek.closingBalance < 0 ? 'danger' : 'warn',
      title: 'Cash low point',
      text: `Lowest closing cash lands in the week of ${worstWeek.weekStart} at ${currencySymbol(summary.reportingCurrency)} ${worstWeek.closingBalance.toLocaleString('en-GB', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}.`,
  ...
  commentary.push({
    ...
    text: `Retention still locked totals ${currencySymbol(summary.reportingCurrency)} ${summary.retentionLocked.toLocaleString('en-GB', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}, with forecast funding fees of ${currencySymbol(summary.reportingCurrency)} ${summary.fundingFeesForecast.toLocaleString('en-GB', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}.`,
  });

  if (strongestExposure) {
    commentary.push({
      tone: strongestExposure.shareOfExposure >= concentrationWarningPercent ? 'warn' : 'ok',
      title: 'Customer concentration',
      text: `${strongestExposure.customerName} represents ${strongestExposure.shareOfExposure}% of receivables in view, worth ${currencySymbol(summary.reportingCurrency)} ${strongestExposure.totalExposure.toLocaleString('en-GB', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}.`,
    });
  }

  if (summary.forecastMinimumCash < 0) {
    warnings.push({
      tone: 'danger',
      title: 'Negative cash forecast',
      text: `The 13-week forecast drops below zero, reaching ${currencySymbol(summary.reportingCurrency)} ${summary.forecastMinimumCash.toLocaleString('en-GB', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}.`,
      weekStart: worstWeek?.weekStart || '',
      action: 'Review payroll timing, invoice issue dates, and funding rules.',
    });
  } else if (summary.forecastMinimumCash < minimumCashBuffer) {
    warnings.push({
      tone: 'warn',
      title: 'Cash buffer below target',
      text: `Lowest forecast cash falls below the configured buffer of ${currencySymbol(summary.reportingCurrency)} ${minimumCashBuffer.toLocaleString('en-GB', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}.`,
      weekStart: worstWeek?.weekStart || '',
      action: 'Consider pulling receipts forward or adding a temporary funding adjustment.',
    });
  }

  if (payrollCoverWeeks > 0 && payrollCoverWeeks < payrollCoverWarningWeeks) {
    warnings.push({
      tone: payrollCoverWeeks < 1 ? 'danger' : 'warn',
      title: 'Payroll cover is thin',
      text: `Current cash covers roughly ${payrollCoverWeeks} weeks of payroll and contractor outflow against a target of ${payrollCoverWarningWeeks} weeks.`,
      action: 'Check the next payroll cycle against funded advances and debtor releases.',
    });
  }

  if (overdueReceivables > 0) {
    warnings.push({
      tone: overdueReceivables > (summary.totalInflows || 0) * 0.15 ? 'danger' : 'warn',
      title: 'Overdue receivables are building',
      text: `${currencySymbol(summary.reportingCurrency)} ${overdueReceivables.toLocaleString('en-GB', { minimumFractionDigits: 0, maximumFractionDigits: 0 })} of QBO receivables are already overdue.`,
      action: 'Chase debtor receipts or reflect a more conservative receipt profile.',
    });
  }
  ...
  if (overduePayables > 0) {
    warnings.push({
      ...
      text: `${currencySymbol(summary.reportingCurrency)} ${overduePayables.toLocaleString('en-GB', { minimumFractionDigits: 0, maximumFractionDigits: 0 })} of supplier liabilities are already overdue in QBO.`,
      action: 'Sense-check supplier timing against payroll and funding needs.',
    });
  }

  if (strongestExposure && strongestExposure.shareOfExposure >= concentrationWarningPercent) {
    warnings.push({
      tone: strongestExposure.shareOfExposure >= 50 ? 'danger' : 'warn',
      title: 'Receipts are concentrated',
      text: `${strongestExposure.customerName} accounts for ${strongestExposure.shareOfExposure}% of receivable exposure in the horizon.`,
      action: 'Stress test receipt delays for this customer before relying on the base case.',
    });
  }
  ...
  if (scenario.key !== 'base') {
    warnings.push({
      tone: 'warn',
      title: `Scenario lens: ${scenario.label}`,
      text: scenario.description,
      action: 'Switch back to Base to compare against the working forecast.',
    });
  }
  ...
  return {
    ...
    payroll: {
      averageWeeklyPayroll: roundMoney(averageWeeklyPayroll),
      nextFourWeekPayroll: roundMoney(nextFourWeekPayroll),
      payrollCoverWeeks: roundMoney(payrollCoverWeeks),
      warningThreshold: roundMoney(payrollCoverWarningWeeks),
    },
  };
}
...
function buildCashflowForecast(input = {}) {
  ...
  const scenario = getScenarioPresetConfig(input.scenarioPreset ?? input.scenario_preset ?? assumptions.scenario_preset);
  ...
  if (assumptions.include_qbo_purchases_actuals !== false) {
  ...
  if (assumptions.include_qbo_open_invoices !== false) {
    qboInvoices.forEach((invoice) => {
      const balance = toNumber(invoice.balance_amount ?? invoice.balanceAmount, 0);
      if (balance <= 0) return;
      const dueDate = normalizeDate(invoice.due_date ?? invoice.dueDate) || normalizeDate(invoice.txn_date ?? invoice.txnDate);
      addExposureEntry(exposureMap, invoice.customer_name ?? invoice.customerName, convertToReporting(balance, invoice.currency, assumptions), {
        overdue: dueDate ? compareDates(dueDate, normalizeDate(new Date().toISOString())) < 0 : false,
        funded: invoice.funded === true,
        currency: invoice.currency,
        nextReceiptDate: dueDate,
      });
  ...
  invoicePlans.forEach((invoice) => {
    ...
    addExposureEntry(exposureMap, invoice.customer_name ?? invoice.customerName, convertToReporting(
      deriveGrossAmount({
        grossAmount: invoice.gross_amount ?? invoice.grossAmount,
        netAmount: invoice.net_amount ?? invoice.netAmount,
        vatAmount: invoice.vat_amount ?? invoice.vatAmount,
        vatTreatment: invoice.vat_treatment ?? invoice.vatTreatment,
      ...
      funded: invoice.funded === true,
      currency: invoice.currency,
      nextReceiptDate: invoice.expected_payment_date ?? invoice.expectedPaymentDate,
    });
    const result = addInvoiceCashLines({
      invoice,
      fundingRules,
  ...
  overheads.forEach((row) => {
    ...
    const payrollMultiplier = isPayrollCategory(row.category) ? toNumber(scenario.payrollMultiplier, 1) : 1;
    const amount = convertToReporting(roundMoney(toNumber(row.amount, 0) * payrollMultiplier), row.currency, assumptions);
    if (amount <= 0) return;
  ...
  const insights = summariseInsights({
    weeks,
    summary,
    assumptions,
    qboInvoices,
    qboBills,
    exposureEntries: finalizeExposureEntries(exposureMap),
    scenario,
  });

  summary.overdueReceivables = insights.overdueReceivables;
  summary.overduePayables = insights.overduePayables;
  summary.payrollCoverWeeks = insights.payroll.payrollCoverWeeks;
  summary.largestCustomerShare = insights.exposureEntries[0]?.shareOfExposure || 0;
