(function (root, factory) {
  ...
  function buildReconciliationSummary(statement, openingBalance) {
    ...
    const variance = roundMoney(importedTotal - enteredOpeningBalance);
    const scaleFactor = reconciliationMode === 'scale_to_opening_balance' && importedTotal
  ...
  function materialiseImportedStatement(input, options) {
    ...
    const warnings = uniqueStrings((draft.warnings || []).concat(rowSummary.warnings));
    ...
    return {
      ...
      importedTotal: rowSummary.importedTotal,
      scheduledReceiptTotal: rowSummary.scheduledReceiptTotal,
  ...
  function parsePdfHeuristicLine(line, index) {
    ...
    if (!raw) return null;
    const dateMatches = raw.match(/\b\d{1,2}[\/.\-]\d{1,2}[\/.\-]\d{2,4}\b|\b\d{4}-\d{2}-\d{2}\b/g) || [];
    const amountMatches = raw.match(/(?:GBP|EUR|£|€)?\s*-?\(?\d{1,3}(?:[,\s]\d{3})*(?:\.\d{2})?\)?/g) || [];
