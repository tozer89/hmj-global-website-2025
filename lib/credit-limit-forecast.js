(function (root, factory) {
  ...
  function sanitizeImportedStatementAdjustmentLines(lines) {
    return (Array.isArray(lines) ? lines : [])
      .map(function (line, index) {
        return {
          id: String((line && line.id) || ('import-adjustment-' + index)),
          include: line && line.include !== false,
          date: normaliseOptionalDate(line && line.date) || isoToday(),
          amount: roundMoney(toNumber(line && line.amount, 0)),
          note: String((line && line.note) || '').trim(),
          warnings: Array.isArray(line && line.warnings)
            ? line.warnings.map(function (entry) { return String(entry || '').trim(); }).filter(Boolean).slice(0, 8)
            : [],
          warningText: String((line && line.warningText) || '').trim(),
        };
      })
      .filter(function (line) {
        return line.date || line.amount || line.note;
      });
  }

  function summarizeImportedStatementAdjustments(lines) {
    const included = (Array.isArray(lines) ? lines : []).filter(function (line) { return line.include !== false; });
    return {
      adjustmentLineCount: Array.isArray(lines) ? lines.length : 0,
      adjustmentIncludedCount: included.length,
      adjustmentTotal: roundMoney(included.reduce(function (total, line) {
        return total + roundMoney(toNumber(line && line.amount, 0));
      }, 0)),
    };
  }
  ...
  function sanitizeImportedStatement(value, fallbackCurrency) {
    ...
    const adjustmentLines = sanitizeImportedStatementAdjustmentLines(source.adjustmentLines);
    const summary = summarizeImportedStatement(rows, fallbackCurrency);
    const adjustmentSummary = summarizeImportedStatementAdjustments(adjustmentLines);
    const importedTotal = source.importedTotal != null ? roundMoney(toNumber(source.importedTotal, summary.importedTotal)) : summary.importedTotal;
    const adjustmentTotal = source.adjustmentTotal != null ? roundMoney(toNumber(source.adjustmentTotal, adjustmentSummary.adjustmentTotal)) : adjustmentSummary.adjustmentTotal;
    const reconciliationTotal = source.reconciliationTotal != null
      ? roundMoney(toNumber(source.reconciliationTotal, importedTotal + adjustmentTotal))
      : roundMoney(importedTotal + adjustmentTotal);
    const scheduledReceiptTotal = source.scheduledReceiptTotal != null
      ? roundMoney(toNumber(source.scheduledReceiptTotal, summary.scheduledReceiptTotal + adjustmentTotal))
      : roundMoney(summary.scheduledReceiptTotal + adjustmentTotal);
    return {
      ...
      importedTotal: importedTotal,
      scheduledReceiptTotal: scheduledReceiptTotal,
      adjustmentLineCount: source.adjustmentLineCount != null ? toWholeNumber(source.adjustmentLineCount, adjustmentSummary.adjustmentLineCount) : adjustmentSummary.adjustmentLineCount,
      adjustmentIncludedCount: source.adjustmentIncludedCount != null ? toWholeNumber(source.adjustmentIncludedCount, adjustmentSummary.adjustmentIncludedCount) : adjustmentSummary.adjustmentIncludedCount,
  ...
  function openingBalanceContext(model) {
    ...
    const adjustmentTotal = roundMoney(statement.adjustmentTotal);
    const reconciliationTotal = roundMoney(statement.reconciliationTotal != null ? statement.reconciliationTotal : (importedTotal + adjustmentTotal));
    ...
    const scheduleScaleFactor = reconciliationMode === 'scale_to_opening_balance' && reconciliationTotal
    ...
    return {
      ...
      adjustmentTotal: adjustmentTotal,
      reconciliationTotal: reconciliationTotal,
      variance: roundMoney(reconciliationTotal - enteredOpeningBalance),
  ...
  function buildImportedOpeningBalanceSchedule(model, weeks, context) {
    ...
    return statement.rows
      ...
      .concat((Array.isArray(statement.adjustmentLines) ? statement.adjustmentLines : [])
        .filter(function (line) { return line.include !== false && roundMoney(line.amount) !== 0; })
        .map(function (line, index) {
          const lineDate = parseDate(line.date || model.forecastStartDate);
          const scaledAmount = roundMoney(line.amount * openingContext.scheduleScaleFactor);
          return {
            id: line.id || ('opening-import-adjustment-' + index),
            date: formatDate(lineDate),
            dateLabel: formatLongDate(lineDate),
            dueDate: formatDate(lineDate),
            dueDateLabel: formatLongDate(lineDate),
            statementDueDate: '',
            statementDueDateLabel: '',
            amount: scaledAmount,
            receiptWeekIndex: findWeekIndexForDate(lineDate, weeks),
            source: 'opening_balance_import_adjustment',
            sourceLabel: 'Imported statement adjustment',
            invoiceRef: '',
            note: line.note || 'Opening-balance reconciliation adjustment',
  ...
  function buildForecast(input, options) {
    ...
    const peakRow = rows.reduce(function (best, row) {
    ...
    const result = {
      ...
      metrics: {
        ...
        peakOverLimit: roundMoney(Math.max(0, peakBalance - model.creditLimit)),
        peakWeekNumber: peakRow ? peakRow.weekNumber : null,
        peakWeekCommencing: peakRow ? peakRow.weekCommencing : '',
  ...
  function buildCurrentScenarioCapacityDelta(assumptions, components, deltaContractors) {
    const currentAdditional = Math.max(0, toWholeNumber(assumptions && assumptions.contractor && assumptions.contractor.additionalContractors, 0));
    const delta = toWholeNumber(deltaContractors, 0);
    if (delta >= 0) {
      return buildCapacityVariant(assumptions, components, {
        additionalContractors: currentAdditional + delta,
        contractorsRemoved: 0,
      });
    }

    const removals = Math.abs(delta);
    if (removals <= currentAdditional) {
      return buildCapacityVariant(assumptions, components, {
        additionalContractors: currentAdditional - removals,
        contractorsRemoved: 0,
      });
    }
    ...
    return buildCapacityVariant(assumptions, components, {
      additionalContractors: 0,
      contractorsRemoved: removals - currentAdditional,
    });
  }
  ...
  function analyseCapacity(input) {
    ...
    if (currentForecast.overallStatus === 'over_limit') {
      const maxRemovalSearch = Math.max(
        assumptions.contractor.currentContractors + assumptions.contractor.additionalContractors,
        1
      );
      for (let removal = 1; removal <= maxRemovalSearch; removal += 1) {
        const forecast = buildCurrentScenarioCapacityDelta(assumptions, components, removal * -1);
        if (forecast.overallStatus !== 'over_limit') {
    ...
    const plusOneForecast = buildCurrentScenarioCapacityDelta(assumptions, components, 1);
    const minusOneForecast = buildCurrentScenarioCapacityDelta(assumptions, components, -1);
    const peakOverLimit = roundMoney(Math.max(0, currentForecast.metrics.forecastPeakBalance - assumptions.creditLimit));
    const plusOnePeakImpact = roundMoney(Math.max(0, plusOneForecast.metrics.forecastPeakBalance - currentForecast.metrics.forecastPeakBalance));
    const minusOnePeakRelief = roundMoney(Math.max(0, currentForecast.metrics.forecastPeakBalance - minusOneForecast.metrics.forecastPeakBalance));
    const contractorEquivalentBasis = minusOnePeakRelief > 0 ? minusOnePeakRelief : plusOnePeakImpact;
    ...
    return {
      ...
      maxSafeWeeklyGrossIncrease: safeWeeklyIncrease,
      currentPeakOverLimit: peakOverLimit,
      contractorEquivalentExcess: contractorEquivalentBasis > 0 && peakOverLimit > 0
        ? roundMoney(peakOverLimit / contractorEquivalentBasis)
        : 0,
      plusOneForecast: plusOneForecast,
      plusOnePeakImpact: plusOnePeakImpact,
      minusOneForecast: minusOneForecast,
      minusOnePeakRelief: minusOnePeakRelief,
  ...
  function generateFallbackSummary(result, input, capacity) {
    ...
    if (forecast.overallStatus === 'over_limit') {
      ...
      const overLimitText = metrics.peakOverLimit
        ? (' The scenario peaks ' + formatCurrency(metrics.peakOverLimit, currency) + ' over limit in week ' + (metrics.peakWeekNumber || '?') + '.')
        : '';
      ...
      return clientName + ' moves over the insured limit under this forecast. The first breach lands in week '
        ...
        + formatCurrency(metrics.creditLimit || 0, currency) + '.' + overLimitText + ' The model assumes ' + termsLabel + ', '
