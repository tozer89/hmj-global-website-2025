...
const DEFAULT_DOC_LEGACY_TIMEOUT_MS = 45000;
...
let pdfParseWorkerModule = null;
let pdfParseWorkerLoadError = null;
let pdfParseWorkerLoadAttempted = false;
let pdfParseModuleInitialised = false;
let pdfParseWorkerConfigured = false;

function initialiseMammothModule() {
  mammothLoadAttempted = true;
  try {
    mammothModule = require('mammoth');
    mammothLoadError = null;
  } catch (error) {
    mammothModule = null;
...
initialiseMammothModule();

function getMammoth() {
  if (!mammothModule && mammothLoadAttempted) {
    initialiseMammothModule();
  }
...
function textutilAvailable() {
  try {
    return fs.existsSync(TEXTUTIL_PATH);
  } catch {
    return false;
  }
}

function shouldRetryPdfParseBootstrap(error) {
  const summary = getLoadErrorSummary(error);
  if (!summary) return false;
  return /(DOMMatrix|Path2D|ImageData|OffscreenCanvas|canvas)/i.test(summary);
}

function loadPdfParseWorkerModule() {
  if (pdfParseWorkerModule || pdfParseWorkerLoadAttempted) {
    return pdfParseWorkerModule;
  }

  pdfParseWorkerLoadAttempted = true;
  try {
    pdfParseWorkerModule = require('pdf-parse/worker');
    pdfParseWorkerLoadError = null;
  } catch (error) {
    pdfParseWorkerLoadError = error;
  }

  return pdfParseWorkerModule;
}

function configurePdfParseWorker(moduleOverride = pdfParseModule) {
  if (pdfParseWorkerConfigured || !moduleOverride) return;

  const PDFParse = moduleOverride && moduleOverride.PDFParse;
  if (!PDFParse || typeof PDFParse.setWorker !== 'function') return;

  const workerModule = loadPdfParseWorkerModule();
  if (!workerModule) return;

  const workerCandidates = [
    typeof workerModule.getData === 'function' ? trimString(workerModule.getData()) : '',
    typeof workerModule.getPath === 'function' ? trimString(workerModule.getPath()) : '',
  ].filter(Boolean);
...
function loadPdfParseModule(options = {}) {
  if (options.preloadWorker) {
    loadPdfParseWorkerModule();
  }
...
function initialisePdfParseModule() {
  if (pdfParseModule) {
    configurePdfParseWorker(pdfParseModule);
    return pdfParseModule;
  }

  if (pdfParseModuleInitialised) {
    return pdfParseModule;
  }

  pdfParseModuleInitialised = true;
  loadPdfParseModule();

  if (!pdfParseModule && shouldRetryPdfParseBootstrap(pdfParseLoadError)) {
    loadPdfParseModule({ preloadWorker: true });
  }
...
function getPdfDependencyLoadSummary() {
  const issues = [];
  const parserSummary = getLoadErrorSummary(pdfParseLoadError);
  if (parserSummary) issues.push(parserSummary);
  const workerSummary = getLoadErrorSummary(pdfParseWorkerLoadError);
  if (workerSummary) issues.push(`worker bootstrap ${workerSummary}`);
  return issues.join(' ');
}

function buildPdfParserUnavailableMessage() {
  const summary = getPdfDependencyLoadSummary();
  return `PDF extraction dependency is unavailable on the server.${summary ? ` ${summary}` : ''}${getPdfRuntimeCompatibilityNote()}`;
}
...
async function inspectPdfBuffer(buffer, options = {}) {
  ...
  if (!PDFParse) {
    throw coded(
      500,
      buildPdfParserUnavailableMessage(),
...
async function extractLegacyDocText(buffer, options = {}) {
  if (!textutilAvailable()) {
    throw coded(
      500,
      'Legacy DOC extraction requires /usr/bin/textutil on this Mac.',
      'legacy_doc_extraction_unavailable'
    );
  }

  const timeoutMs = getPositiveTimeout(
    'CANDIDATE_MATCH_DOC_TIMEOUT_MS',
    Number(options.timeoutMs) || DEFAULT_DOC_LEGACY_TIMEOUT_MS
  );
  const tempDir = await fsPromises.mkdtemp(path.join(os.tmpdir(), 'hmj-legacy-doc-'));
  const inputPath = path.join(tempDir, 'candidate.doc');
  const parseStartedAt = Date.now();

  try {
    await fsPromises.writeFile(inputPath, buffer);
    logMatcher('info', 'Starting legacy DOC parse', {
      parser: 'textutil',
      bytes: buffer?.byteLength || 0,
      timeoutMs,
      startedAt: new Date().toISOString(),
    ...
    const result = await execFileAsync(TEXTUTIL_PATH, ['-convert', 'txt', '-stdout', inputPath], {
      encoding: 'utf8',
      maxBuffer: 12 * 1024 * 1024,
      timeout: timeoutMs,
    });
    const text = normaliseWhitespace(result?.stdout || '');

    logMatcher('info', 'Legacy DOC parse completed', {
      bytes: buffer?.byteLength || 0,
      durationMs: Date.now() - parseStartedAt,
    ...
    return {
      ...
      rawText: text,
      parser: 'textutil',
      strategy: 'legacy_doc_textutil',
      attempts: [{
        strategy: 'legacy_doc_textutil',
        durationMs: Date.now() - parseStartedAt,
        rawTextLength: text.length,
        normalizedTextLength: text.length,
        alphaNumericCount: countAlphaNumericCharacters(text),
        wordCount: countWordTokens(text),
        textUsable: !!text,
        usabilityCode: text ? 'ok' : 'no_text',
      }],
    };
  } catch (error) {
    const message = trimString(error?.stderr || error?.message || String(error));
    if (error?.code === 'ETIMEDOUT' || /timed out/i.test(message)) {
      throw coded(
        504,
        `Legacy DOC extraction timed out after ${timeoutMs}ms.`,
        'doc_extraction_timeout',
        {
          details: {
            stage: 'extraction',
            parser: 'doc',
    ...
    throw coded(
      500,
      `Legacy DOC extraction failed via textutil.${message ? ` ${message}` : ''}`,
  ...
  for (const document of documents) {
    ...
    try {
      ...
      if (document.extension === 'pdf') {
        ...
        extractionResult = await extractLegacyDocText(document.buffer, { timeoutMs: docxTimeoutMs });
      }
