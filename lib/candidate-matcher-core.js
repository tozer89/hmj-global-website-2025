...
function getMammoth() {
  if (mammothModule === undefined) {
    mammothModule = optionalRequire('mammoth');
  }
...
function getPdfParseModule() {
  if (pdfParseModule === undefined) {
    pdfParseModule = optionalRequire('pdf-parse');
  }
...
function buildStorageKey(runId, filename) {
  ...
  const stem = slugify(filename.replace(/\.[^.]+$/, '')) || `document-${Date.now()}`;
...
async function extractPdfText(buffer) {
  ...
  if (!PDFParse) {
    throw coded(500, 'PDF extraction dependency is unavailable on the server.', 'pdf_parser_unavailable');
  }
...
async function extractDocxText(buffer) {
  ...
  if (!mammoth || typeof mammoth.extractRawText !== 'function') {
    throw coded(500, 'DOCX extraction dependency is unavailable on the server.', 'docx_parser_unavailable');
  }
  const result = await mammoth.extractRawText({ buffer });
  return normaliseWhitespace(result?.value || '');
}

async function extractCandidateDocuments(documents) {
  const processed = [];
  ...
  for (const document of documents) {
    ...
    try {
      ...
      if (document.extension === 'pdf') {
        text = await extractPdfText(document.buffer);
      } else if (document.extension === 'docx') {
        text = await extractDocxText(document.buffer);
...
async function fetchPublishedLiveJobs(supabase) {
...
async function callOpenAIForMatch(payload, options = {}) {
  ...
  const timeoutMs = getEnvNumber('OPENAI_MATCH_TIMEOUT_MS', 90000);
  ...
  const requestBody = {
    ...
    max_output_tokens: getEnvNumber('OPENAI_CANDIDATE_MATCH_MAX_OUTPUT_TOKENS', 5000),
    text: {
      format: {
        type: 'json_schema',
  ...
  for (let index = 0; index < models.length; index += 1) {
    ...
    try {
      ...
      if (error?.name === 'AbortError') {
        throw coded(504, 'OpenAI candidate matching timed out.', 'openai_timeout');
      }
      lastError = error;
    } finally {
...
async function maybeStoreUploads({ supabase, documents, runId, userEmail, shouldStore }) {
  if (!shouldStore) {
  ...
  for (const document of storable) {
    ...
    const upload = await supabase.storage.from(bucket).upload(storageKey, document.buffer, {
...
async function saveMatchRun({ supabase, runId, actorEmail, recruiterNotes, extraction, analysisResult, documents, bucket, liveJobs, enabled }) {
  if (!enabled) {
  ...
  const files = await saveMatchFiles({ supabase, runId, documents, bucket });
