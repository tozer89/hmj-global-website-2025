...
const MIN_MEANINGFUL_EXTRACTED_TEXT_CHARS = 10;
const MIN_MEANINGFUL_EXTRACTED_TEXT_WORDS = 3;
...
function assessExtractedText(text, options = {}) {
  ...
  const wordCount = (cleanedText.match(/[A-Za-z0-9][A-Za-z0-9+'’&./-]*/g) || []).length;
  return {
    parser,
    rawText,
    cleanedText,
    ...
    alphaNumericCount,
    wordCount,
    onlyArtifacts: !!rawText && !cleanedText,
    meaningful: !!cleanedText
      && alphaNumericCount >= MIN_MEANINGFUL_EXTRACTED_TEXT_CHARS
      && wordCount >= MIN_MEANINGFUL_EXTRACTED_TEXT_WORDS,
  };
}
...
function buildNoReadableTextMessage(document, assessment = {}) {
  if (trimString(document?.extension) === 'pdf') {
    if (assessment.onlyArtifacts) {
      return 'This PDF appears to contain only page markers or other non-CV artifacts. Please upload a text-based PDF or Word document.';
    }
    return 'This PDF appears to contain little or no extractable text. It may be scanned, image-only, or missing a text layer. Please upload a text-based PDF or Word document.';
  }
  return 'No readable text could be extracted from this document.';
}

function buildExtractionFailureMessage(document, error) {
  const extension = trimString(document?.extension);
  const rawMessage = trimString(error?.message) || 'Extraction failed unexpectedly.';
  if (extension !== 'pdf') return rawMessage;
  if (error?.code === 'pdf_parser_unavailable') {
    return 'PDF extraction is not available on this server right now. Please try again later or upload a Word document.';
  }
  if (error?.code === 'pdf_extraction_timeout') {
    return 'PDF text extraction timed out before it completed. Please try a smaller text-based PDF or upload a Word document.';
  }
  if (/password/i.test(rawMessage)) {
    return 'This PDF is password-protected and could not be parsed. Please remove the password or upload a Word document.';
  }
  if (/invalid pdf|formaterror|xref|startxref|unexpected eof|missing pdf|malformed/i.test(rawMessage)) {
    return 'This PDF could not be parsed. It may be corrupted or unsupported. Please export it again as a text-based PDF or upload a Word document.';
  }
  return 'This PDF could not be parsed by the matcher. Please export it again as a text-based PDF or upload a Word document.';
}

function summariseNoReadableTextFailure(documents) {
  const failedDocuments = (Array.isArray(documents) ? documents : [])
    .filter((document) => trimString(document?.status) !== 'ok');
  if (!failedDocuments.length) {
    return 'None of the uploaded documents produced readable candidate text. Review the per-file extraction details and try again.';
  }
  const onlyPdfFailures = failedDocuments.every((document) => trimString(document?.extension) === 'pdf');
  if (!onlyPdfFailures) {
    return 'None of the uploaded documents produced readable candidate text. Review the per-file extraction details and try again.';
  }
  if (failedDocuments.every((document) => /password-protected/i.test(trimString(document?.error)))) {
    return failedDocuments.length === 1
      ? 'The uploaded PDF is password-protected and could not be parsed. Please remove the password or upload a Word document.'
      : 'All uploaded PDFs are password-protected and could not be parsed. Please remove the passwords or upload Word documents.';
  }
  if (failedDocuments.every((document) => /little or no extractable text|only page markers/i.test(trimString(document?.error)))) {
    return failedDocuments.length === 1
      ? 'The uploaded PDF appears to contain little or no extractable text. Please upload a text-based PDF or Word document.'
      : 'The uploaded PDFs appear to contain little or no extractable text. Please upload text-based PDFs or Word documents.';
  }
  if (failedDocuments.every((document) => /could not be parsed/i.test(trimString(document?.error)))) {
    return failedDocuments.length === 1
      ? 'The uploaded PDF could not be parsed. Please export it again as a text-based PDF or upload a Word document.'
      : 'The uploaded PDFs could not be parsed. Please export them again as text-based PDFs or upload Word documents.';
  }
  return 'None of the uploaded documents produced readable candidate text. Review the per-file extraction details and try again.';
}
...
async function extractCandidateDocuments(documents, options = {}) {
  ...
  for (const document of documents) {
    ...
    try {
      ...
      const assessment = assessExtractedText(text, { parser: document.extension });
      const requiresMeaningfulTextCheck = document.extension === 'pdf';
      if (!assessment.cleanedText || (requiresMeaningfulTextCheck && !assessment.meaningful)) {
        logMatcher('warn', 'Candidate extraction produced insufficient readable text', {
          ...describeDocumentForLogs(document),
          rawTextLength: assessment.rawLength,
          cleanedTextLength: assessment.cleanedLength,
          alphaNumericCount: assessment.alphaNumericCount,
          wordCount: assessment.wordCount,
          onlyArtifacts: assessment.onlyArtifacts,
        });
        processed.push({
          ...document,
          status: 'failed',
          error: buildNoReadableTextMessage(document, assessment),
        });
        continue;
      }
      ...
      logMatcher('info', 'Candidate extraction succeeded', {
        ...describeDocumentForLogs(document),
        extractedTextLength: assessment.cleanedLength,
        rawTextLength: assessment.rawLength,
      ...
      logMatcher('error', 'Candidate extraction failed', {
        ...describeDocumentForLogs(document),
        parser: document.extension || document.parserPath || '',
      ...
      processed.push({
        ...document,
        status: 'failed',
        error: buildExtractionFailureMessage(document, error),
      });
    }
  }
...
function buildPreparedRunPayload({ runId, actorEmail, recruiterNotes, extraction, documents, bucket }) {
  ...
  const noReadableTextMessage = summariseNoReadableTextFailure(extraction.documents);
