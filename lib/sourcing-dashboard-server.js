...
async function handleApi(request, response, workflowRoot, routePath, requestUrl) {
  ...
  const importMatch = routePath.match(/^\/api\/roles\/([^/]+)\/import$/);
  if (importMatch && request.method === 'POST') {
    const roleId = decodeURIComponent(importMatch[1]);
    const body = safeJsonParse(await readRequestBody(request));
    const fileName = trimString(body.fileName, 240) || 'uploaded-candidates.csv';
    const text = typeof body.text === 'string' ? body.text : '';
    const postImportAction = trimString(body.postImportAction, 80) || 'run_preview_triage';
    const importResult = core.importPreviewCandidatesFromText({
      workflowRoot,
      roleId,
      fileName,
      text,
    });
    const role = postImportAction === 'import_only'
      ? core.summariseRoleFromDisk(workflowRoot, roleId)
  ...
  const bulkCvMatch = routePath.match(/^\/api\/roles\/([^/]+)\/bulk-cv-upload$/);
  if (bulkCvMatch && request.method === 'POST') {
    const roleId = decodeURIComponent(bulkCvMatch[1]);
    const body = safeJsonParse(await readRequestBody(request));
    const files = Array.isArray(body.files) ? body.files : [];
    const postImportAction = trimString(body.postImportAction, 80) || 'review_downloaded_cvs';
    const bulkImportResult = await core.importBulkCvFiles({
      workflowRoot,
      roleId,
      files,
    });
    const role = postImportAction === 'upload_only'
      ? core.summariseRoleFromDisk(workflowRoot, roleId)
      : await core.runRoleWorkspace({
        workflowRoot,
        roleId,
